...
public class ArgumentOrgCommandReceiver implements ExternalArgumentReceiver<Organization> {
    ...
    public boolean receiveCommand(CommandDescription command, String[] args) throws BuildObjectException {
        ...
        CommandStatusResponse response = new ArgumentRequestSender<Organization>().sendCommand(command, args, organization, ServerConnectionHandler.getCurrentConnection());
        if (response != null) {
