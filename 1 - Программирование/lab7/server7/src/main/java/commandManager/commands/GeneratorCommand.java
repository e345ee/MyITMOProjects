    ...
    public void execute(String[] args) {
        ...
        if (Integer.parseInt(args[1])<0 || Integer.parseInt(args[1])>100 ){
            ...
            for (int i = 0; i < Integer.parseInt(args[1]); i++){
                ...
                Product newPr = generateProduct();
                PostgresSQLManager manager = new PostgresSQLManager();
                ArrayList<Integer> ids = manager.writeObjectToDatabase(newPr, client);
                int generatedId =  ids.get(1);

                if (generatedId != -1 && ids.get(0) != -1) {

                    newPr.setId(generatedId);
                    newPr.getManufacturer().setId(Long.valueOf(ids.get(0)));
                    productHandler.addElementToCollection(newPr);
