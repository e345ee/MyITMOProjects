...
static inline int pair_diff(char *array, int bi) {
  return bit_isset(array, pair_index(bi));
}

static inline void pair_toggle(char *array, int bi) {
  int pi = pair_index(bi);
  if (bit_isset(array, pi))
    bit_clear(array, pi);
  else
    bit_set(array, pi);
}
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (pair_diff(bd_sizes[k].alloc, bi)) {
      break;
    }
...
static int bd_add_pair_free_in_range(int k, int i, void *bd_left, void *bd_right) {
  if (!pair_diff(bd_sizes[k].alloc, i))
    return 0;

  int j = i ^ 1;
  char *ai = (char *)addr(k, i);
  char *aj = (char *)addr(k, j);

  if (ai >= (char *)bd_left && ai + BLK_SIZE(k) <= (char *)bd_right) {
    lst_push(&bd_sizes[k].free, ai);
    return BLK_SIZE(k);
  }
  if (aj >= (char *)bd_left && aj + BLK_SIZE(k) <= (char *)bd_right) {
    lst_push(&bd_sizes[k].free, aj);
    return BLK_SIZE(k);
  }
  return 0;
}
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {
    ...
    int right = blk_index(k, (char*)bd_right - 1);
    

    free += bd_add_pair_free_in_range(k, left, bd_left, bd_right);
    if (right > left)
      free += bd_add_pair_free_in_range(k, right, bd_left, bd_right);
  }
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    int npairs = (NBLK(k) + 1) / 2;         
    sz = sizeof(char) * ROUNDUP(npairs, 8) / 8;
