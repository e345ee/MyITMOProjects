...
main(int argc, char *argv[])
{
  ...
  if(pipe(lock) < 0){
    fprintf(2, "tsigstop: pipe failed\n");
    exit(1);
  }

  if(write(lock[1], "X", 1) != 1){
    fprintf(2, "tsigstop: initial token write failed\n");
    exit(1);
  }
  ...
  printf("[parent %d] let the child run for a while...\n", mypid);
  lock_release(lock, "parent");

  sleep(100);

  lock_acquire(lock, "parent");
  printf("[parent %d] sending SIGSTOP to child (kill2 %d %d)\n",
         mypid, pid, SIGSTOP);
  lock_release(lock, "parent");
  ...
  lock_acquire(lock, "parent");
  printf("[parent %d] child should be silent now\n", mypid);
  lock_release(lock, "parent");

  sleep(100);

  lock_acquire(lock, "parent");
  printf("[parent %d] sending SIGCONT to child (kill2 %d %d)\n",
         mypid, pid, SIGCONT);
  lock_release(lock, "parent");
  ...
  lock_acquire(lock, "parent");
  printf("[parent %d] child should start printing again\n", mypid);
  lock_release(lock, "parent");
  ...
  lock_acquire(lock, "parent");
  printf("[parent %d] sending SIGKILL to child\n", mypid);
  printf("[parent %d] waiting for child to exit\n", mypid);
